...
Six paths are deliberately absent from this tree, and `sync-upstream.sh`
keeps them out (the excluded-paths field of its vendor table):
`skills/docx`, `skills/pdf`, `skills/pptx`, `skills/xlsx`,
`skills/doc-coauthoring` and `.claude-plugin`.

The first five are a rights question. The first four carry a licence that
forbids redistribution; the fifth carries no licence at all. Different
symptoms, one diagnosis: this public repo holds no grant to redistribute
either, and publishing it would be redistribution.

`.claude-plugin` is excluded for a different reason entirely — upstream's own
marketplace manifest has no purpose here and advertises content that is not on
disk. Its section below says why.
...
Unlike the document skills it has no plugin of its own to point at: upstream
lists `doc-coauthoring` under `example-skills`, alongside unrelated material,
so reaching it machine-locally means installing that whole plugin from
`anthropic-agent-skills`. Nothing here enables it.
