...
for entry in "${vendors[@]}"; do
	...
	for path in "${excluded[@]}"; do
		if [[ -e "${prefix}/${path}" ]]; then
			# Fail the run, do not just say so. This is the licence guard: a
			# warning on a zero exit is indistinguishable from success to
			# anything chaining `&& git push`, which is how excluded content
			# would get published on a green status.
